clearvars; close all; clc; warning off;
 ...
 %media lunghzza nuovo path e rapporto rispetto a quella iniziale
 %10 path fissi, esegui a partire da ognuno di questi
 %y lunghezza norm, x tempo
...
mkdir(['/home/cesare/TESI/test_bontaSoluzione/' test_name]);

for j = 1:1:length(start_conf(1,:))
    close all
    load( ['/home/cesare/TESI/Path/path_Start_Goal' num2str(j)],'path_vector');
    
    folder_path = ['/home/cesare/TESI/test_bontaSoluzione/' test_name '/startGoal_' num2str(j)];
    mkdir(folder_path);
    ...
    for i=1:1:length(path_vector)
        ...
        vector = [j i time number_replanning replanned_path_cost cost_original];
