...
for j = 1:length(other_paths)
    ...
    for k=1:length(path2_node_vector)
            ...
            diff_subpath_cost = subpath1_cost-subpath2_cost;  %margine di costo concesso a connecting_path
            distance_path_node = norm(path1_node.q-path2_node.q,2);
            
            if (diff_subpath_cost>0 && distance_path_node<diff_subpath_cost) %se il costo della porzione rimanente di path1 è maggiore di quella di path2 ha senso verificare se muoversi verso path2 è vantaggioso in termini di costo
